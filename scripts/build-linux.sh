...
# Step 3: Build piadina and azdora
build_piadina_azdora() {
  log "3. Build piadina + azdora (Autotools, dynamic build)"

  git_clone_and_checkout "$PIADINA_REPO_URL" "$WORK/piadina-src" "$PIADINA_REF"

  # piadina sources include: #include "../libarchive/libarchive/archive.h"
  if [[ ! -f "$WORK/piadina-src/$LIBARCHIVE_HEADER" ]]; then
    log "Fetch libarchive source tree (headers expected at $LIBARCHIVE_HEADER)"
    rm -rf "$WORK/piadina-src/libarchive"
    git clone --depth 1 "$LIBARCHIVE_REPO" "$WORK/piadina-src/libarchive"
  fi

  validate_file_exists "$WORK/piadina-src/$LIBARCHIVE_HEADER" "libarchive header required for build"
  ...
  (cd "$WORK/piadina-src" && ./configure --disable-static-build)
...
build_bombom() {
  ...
  log "Building bombom with rebar3"
  (cd "$WORK/bombom-src" && rebar3 escriptize)
...
# Step 6: Assemble payload directory
assemble_payload() {
  log "6. Assemble payload directory"
...
# Step 7: Package binary
package_binary() {
  log "7. Package into $OUTPUT"
